            ...
            {!! Form::open([
                ...
                <div class="form-row">
                    <div class="col-md-3 mb-3">
                        <label for="" style="display: block">Type de compte</label>
                        {{-- <select name="profile_type_id" class="profile-select form-control selectpicker"  data-style="btn-primary">
                            @foreach ($data['profile_types'] as $type)
                                <option name="profile_type_id"  value="{{ $type->id }}" data-slug="{{ $type->name }}"  {{ $type->id == $suer->profile_type_id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select> --}}
                        <button class="btn btn-primary" onclick="return false;">{{ $user->profile_type->name }}</button>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="username" class="form-control" id="validationCustomUsername" value="{{ $user->username }}" placeholder="Username" aria-describedby="inputGroupPrepend" required="">
                            <div class="invalid-feedback">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="service_id" class="form-control selectpicker" >
                            @foreach ($data['list_services'] as $service)
                                <option name="service_id"  value="{{ $service->id }}"  {{ $user->service_id == $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-md-3" style="{{ $user->profile_type->name != 'deliveryMan' ? 'display: none' : '' }}">
